...
void dumpJQM(sn_uint32 uProcessID, sn_uint64 uSpuThreadID) {
	...
	pVal = getSpuVal("gPm.mEaJobQueue");
	if (pVal == NULL){
	...
	if (getSpuVal("gPm.mJobDescFreePool.mJobDescAllocatorInfo.poolInfo.eaPoolStart")->val.u32 > 0) {

		printf("\n");
		printf("\t*** Job Descriptor Temporal Pool Info ***\n");
		printf("\tDescSize\tnumPooled\tEA\n");
		for(i = 0; i < 8; i++) {
			printf("\t%d\t\t\t%d/%d\t\t\t%#x...%#x\n",
				(i) ? (i * 128) : 64,
				readSpuUint32Array("gPm.mJobDescFreePool.mNumDesc", i),
				(readSpuUint32Array("gPm.mJobDescFreePool.mJobDescAllocatorInfo.eaDescEnd", i) -
					readSpuUint32Array("gPm.mJobDescFreePool.mJobDescAllocatorInfo.eaDescStart", i)) / ((i) ? (i * 128) : 64),
				readSpuUint32Array("gPm.mJobDescFreePool.mJobDescAllocatorInfo.eaDescStart", i),
				readSpuUint32Array("gPm.mJobDescFreePool.mJobDescAllocatorInfo.eaDescEnd", i));
		}
	}
}
...
void dumpJobQueue(sn_uint32 uProcessID, sn_uint64 uThreadID, sn_address jobQueue) {
	...
	unsigned int numValidJobDescriptorPool;
	...
	/* job descriptor pool */
	numValidJobDescriptorPool = 0;
	for(i = 0; i < 8; i++) {
		if(readPpuUint32Array(jobQueue, (128 * 12) / 4 + i) != 0) {numValidJobDescriptorPool++;}
	}
	if(numValidJobDescriptorPool > 0) {
		printf("\t*** Job Descriptor Pool Info ***\n");
		printf("\tDescSize\tnumAvailable\tEA\n");
		for(i = 0; i < 8; i++) {
			printf("\t%d\t\t\t%d/%d\t\t\t\t%#x...%#x\n",
				   (i) ? (i * 128) : 64,
				   readPpuUint32Array(jobQueue, (128 * 13) / 4 + i * 4 + 1) >> 16,	
				   (readPpuUint32Array(jobQueue, (128 * 12) / 4 + i + 8) - readPpuUint32Array(jobQueue, (128 * 12) / 4 + i)) /
				   ((i) ? (i * 128) : 64),
				   readPpuUint32Array(jobQueue, (128 * 12) / 4 + i),
				   readPpuUint32Array(jobQueue, (128 * 12) / 4 + i + 8));
		}
	}
	...
	while(isEnd == 0) {
		...
		if((command_low & 0x01) == phaseRead) {
			...
			case 0x00:
				...
				break;
			case 0x02:
				/* auto release job command */
				{
					sn_uint32 eaJobDescriptor;
					sn_uint32 eaSemaphore;
					eaJobDescriptor = (command_low & 0xfffffff0UL);
					eaSemaphore = command_high & 0xffffff80UL;
					printf("JOB: Descriptor %#x, Semaphore %#x, Tag %02u (auto release)\n", eaJobDescriptor, eaSemaphore, (command_high >> 3) & 0x0fUL);
				}
				break;
			case 0x03:
				/* extension command */
...
void dumpPort2(sn_uint32 uProcessID, sn_uint64 uThreadID, sn_address port2) {
	sn_uint32	eaBuffer;
	sn_uint32	port2_atomic_high;
	sn_uint32	port2_atomic_low;
	...
	port2_atomic_high = readPpuUint32Array(port2, 4);
	port2_atomic_low = readPpuUint32Array(port2, 5);

	printf("* JobQueuePort2 Info\n");
	printf("\tEA of JobQueuePort2          \t%#x\n", port2.word[0]);
	printf("\tEA of JobQueue               \t%#x\n", readPpuUint32Array(port2, 4 + 2));
	printf("\tJob Count                    \t%u\n", port2_atomic_high);
	printf("\tJobQueue Handle              \t%d\n", readPpuUint32Array(port2, 4 + 3));
	printf("\n\n");
}
